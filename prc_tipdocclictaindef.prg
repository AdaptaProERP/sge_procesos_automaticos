...
PROCE MAIN(uPar)
      ...
      cSqlA:=[ SELECT ]+;
             [ TDC_TIPO ]+;
             [ FROM DPTIPDOCCLI ]+;
             [ INNER JOIN DPDOCCLI ON TDC_TIPO=DOC_TIPDOC AND DOC_TIPTRA="D" AND DOC_ACT=1 ]+;
             [ INNER JOIN DPTIPDOCCLI_CTA ON CIC_CODIGO=TDC_TIPO ]+;
      ...
      IF !oDp:lPanel
         ...
         EJECUTAR("BRTIPDOCCLICTA",cWhere)
      ...
      oErp:cDescri:=LSTR(oErp:nMonto)+"/"+LSTR(nTodos)+" Tipo de Documentos del Cliente"+CRLF+"sin Integración Contable"
